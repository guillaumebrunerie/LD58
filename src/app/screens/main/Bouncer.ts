...
export class Bouncer {
	...
	public remove(): void {
		...
		if (logo) {
			animate(
				...
				.catch((error) => {
					console.error(
						"Error during logo removal animation:",
						error,
					);
				});
		}
	...
	private setLimits(logo: Logo): void {
		...
		if (position.y + top <= this.yMin) {
			direction =
				direction === DIRECTION.NW ? DIRECTION.SW : DIRECTION.SE;
		}
		if (position.y + bottom >= this.yMax) {
			direction =
				direction === DIRECTION.SE ? DIRECTION.NE : DIRECTION.NW;
		}
		if (position.x + left <= this.xMin) {
			direction =
				direction === DIRECTION.NW ? DIRECTION.NE : DIRECTION.SE;
		}
		if (position.x + right >= this.xMax) {
			direction =
				direction === DIRECTION.NE ? DIRECTION.NW : DIRECTION.SW;
		}
